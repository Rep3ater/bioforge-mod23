...
public class BiologicalLimbItem extends Item {
    private final boolean isLeg;
    private BiologicalLimbItem(boolean leg) { super(new Properties().stacksTo(1)); this.isLeg = leg; }
    public static BiologicalLimbItem ofArm() { return new BiologicalLimbItem(false); }
    public static BiologicalLimbItem ofLeg() { return new BiologicalLimbItem(true); }
    ...
    private void attach(ItemStack stack, Player surgeon, LivingEntity target, InteractionHand hand) {
        ...
        bio.setLimb(empty, LimbState.transplanted(donorDna, model));
        bio.removeBlood(0.2f);
        bio.addPain(2f);
        bio.addMedicalHistoryEntry("[T] Biological graft → " + empty
        ...
        surgeon.sendSystemMessage(Component.literal(
            "[BioForge] §aBiological limb grafted: " + empty).withStyle(ChatFormatting.GREEN));
    }
