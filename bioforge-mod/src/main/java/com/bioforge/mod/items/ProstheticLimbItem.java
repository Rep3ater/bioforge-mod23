...
public class ProstheticLimbItem extends Item {
    private final boolean isLeg;
    private ProstheticLimbItem(boolean leg) { super(new Properties().stacksTo(1)); this.isLeg = leg; }
    public static ProstheticLimbItem ofArm() { return new ProstheticLimbItem(false); }
    public static ProstheticLimbItem ofLeg() { return new ProstheticLimbItem(true); }
    ...
    private void attach(ItemStack stack, Player surgeon, LivingEntity target, InteractionHand hand) {
        ...
        if (empty == null) {
            surgeon.sendSystemMessage(Component.literal(
                "[BioForge] No empty " + (isLeg ? "leg" : "arm") + " slot.").withStyle(ChatFormatting.RED));
            return;
        }

        String model = stack.getOrCreateTag().getString("model_override");
        bio.setLimb(empty, LimbState.prosthetic(model));
        bio.addPain(1.5f);
        bio.addMedicalHistoryEntry("[T] Prosthetic → " + empty
        ...
        stack.shrink(1);
        surgeon.sendSystemMessage(Component.literal(
            "[BioForge] §aProsthetic attached: " + empty).withStyle(ChatFormatting.GREEN));
    }
